...
export const CustomBudgetCard = ({ budget, onSelect, onEdit, onDelete }) => {
  ...
  return (
    <motion.div
      ...
      className="w-full bg-[#14171E] hover:bg-[#1A1F29] border border-white/[0.06] hover:border-white/10 rounded-[24px] p-4 transition-colors cursor-pointer group relative shadow-lg"
    >
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-white font-bold text-[15px] leading-snug group-hover:text-[#FF6D1F] transition-colors">
              {name}
            </h3>
            {budget.description ? (
        ...
        <div className="flex items-center gap-1">
          <div className="relative" ref={menuRef}>
            <button
              ...
              className="w-8 h-8 rounded-full flex items-center justify-center text-[#64748B] hover:text-white hover:bg-white/5 transition-colors cursor-pointer"
            >
            ...
            <AnimatePresence>
              {showMenu && (
                <motion.div
                  ...
                  className="absolute right-0 top-9 w-32 bg-[#1A1F29] border border-white/10 rounded-2xl shadow-2xl py-1.5 z-30"
                >
                  <button
                    type="button"
                    onClick={() => {
          ...
          <div className="text-right">
            {isOverBudget ? (
              ...
              <div>
                ...
                <span className="text-[#FF6D1F] font-bold">{formatCurrency(remaining)}</span>
              </div>
            )}
          </div>
        </div>
